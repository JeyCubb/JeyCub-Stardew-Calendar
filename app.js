...
function renderTrackerSubfilters() {
  ...
  if (activeTrackerSheet === 'villagers') {
    ...
    viewToggleWrap.style.marginRight = '0.5rem';
    viewToggleWrap.style.background = 'rgba(0,0,0,0.35)';
    viewToggleWrap.style.borderRadius = '8px';
    viewToggleWrap.style.padding = '2px';
    viewToggleWrap.style.border = '1px solid var(--border-glass)';

    const btnCards = document.createElement('button');
    btnCards.className = `tracker-subfilter-btn ${currentVillagerViewMode === 'cards' ? 'active' : ''}`;
    btnCards.innerText = '📋 Cards';
    btnCards.onclick = () => setVillagerViewMode('cards');

    const btnMap = document.createElement('button');
    btnMap.className = `tracker-subfilter-btn ${currentVillagerViewMode === 'map' ? 'active' : ''}`;
    btnMap.innerText = '🗺️ Live Map';
    btnMap.onclick = () => setVillagerViewMode('map');

    viewToggleWrap.appendChild(btnCards);
    viewToggleWrap.appendChild(btnMap);
    container.appendChild(viewToggleWrap);
  }
...
function renderTrackerGridOnly() {
  ...
  const mapView = document.getElementById('villager-map-view');
  
  if (activeTrackerSheet === 'villagers' && currentVillagerViewMode === 'map') {
    if (grid) grid.style.display = 'none';
    if (mapView) mapView.style.display = 'flex';
    initVillagerMapControls();
    renderVillagerMap();
    return;
  } else {
  ...
  filtered.forEach(item => {
    ...
    if (activeTrackerSheet === 'villagers') {
      ...
      notesText = `
        <div style="line-height: 1.3;"><strong style="color: #60a5fa;">🕒 Schedule:</strong> ${item.schedule}</div>
        <button class="btn-card-view-on-map" onclick="openVillagerOnMap('${item.id}', event)">🗺️ View on Map</button>
      `;
    }

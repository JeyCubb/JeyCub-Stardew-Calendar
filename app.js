...
document.getElementById('form-crop').addEventListener('submit', (e) => {
  ...
  if ((location === 'Main Farm' || location === 'Rail road' || location === 'Railroad' || location === 'Train Station' || location === 'Quarry') && !canGrowInSeason(cropKey, harvestDate.season)) {
  ...
  if (regrowInterval > 0) {
    ...
    if (canRegrow) {
      ...
      for (let i = 0; i < 60; i++) {
        ...
        if ((location === 'Main Farm' || location === 'Rail road' || location === 'Railroad' || location === 'Train Station' || location === 'Quarry') && !canGrowInSeason(cropKey, nextHarvestDate.season)) {
          if (!crop.isTree) {
...
window.closeCropManager = function(event) {
  if (event) {
    event.stopPropagation();
    event.preventDefault();
  }
...
function renderTrackerGridOnly() {
  ...
  const filtered = dataList.filter(item => {
    ...
    if (currentTrackerSearch) {
      ...
      const matchHome = item.home && item.home.toLowerCase().includes(q);
      ...
      if (!matchName && !matchSource && !matchHome && !matchNotes && !matchCategory && !matchDetails && !matchLoved && !matchLiked && !matchSchedule && !matchBirthday && !matchIngredients && !matchBuffs && !matchDesc && !matchLocation && !matchZone && !matchType && !matchBobber && !matchBait && !matchTackle && !matchSchedTable && !matchAltTable && !matchRain && !matchVariations) {
        return false;
      }
    }
